...
const Container = ({ children, content, onDrawRect, rects }) => {
  const svgRef = useRef()
  ...
  const transformCood = ({clientX, clientY}) => {
    ...
    return {x: clientX - clientRect.x + bBox.x,y: clientY - clientRect.y + bBox.y }
  }
...
const createShape = (type) => {
  ...
  const drawRect = (evt) => {
    ...
    pathRef.current = {elem: path, start, index: Array.prototype.indexOf.call(target.parentNode.children, target), startShape }
  }
  return (
    <>
      <g
        ...
        onMouseMove={(evt) => {
          ...
          if (pathRef.current) {
            ...
            elem.setAttributeNS(null, 'd', `M ${start.x},${start.y} L ${end.x},${start.y} L ${end.x},${end.y}, L ${start.x},${end.y} Z`)
          }
        }}
        onMouseUp={(evt) => {
          if (pathRef.current) {
            ...
            svgRef.current.removeChild(pathRef.current.elem)
            onDrawRect(pathRef.current.index, Array.prototype.indexOf.call(target.parentNode.children, target), rectDim)
            pathRef.current = null
          }
        }}
      >
        {children}
        {rects.map(dim => (<rect {...dim} style={{pointerEvents: 'none'}} stroke="#00ff48" fill="none"/>))}
      </g>
      {pos &&
        createPortal(
          <div
