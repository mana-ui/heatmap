  ...
  const handlers = {
    mousedown({target, clientY}) {
      if (target === minRef.current.elem) {
          draggingRef.current = { y: clientY, pos: minPosRef.current, posRef: minPosRef, ref: minRef, idx: 0 };
      } else if (target === maxRef.current.elem) {
          draggingRef.current = { y: clientY, pos: maxPosRef.current, posRef: maxPosRef, ref: maxRef, idx: 1 };
      }
    },
    mousemove(evt) {
      evt.preventDefault();
      if (draggingRef.current) {
        ...
        const pos = Math.max(Math.min(delta + draggingRef.current.posRef.current, max), min);
